...
In an even broader sense the toy example in this project brings up bigger questions, for instance imagine a world in which you had a really good just in time intervention + recommendation system that could prevent someone from being sad. Is this even the right choice? A simple model might say something along the lines the less negative emotions you feel the better your mental health, and there of course is some association there. On the flipside there are times when feeling sad is probably a good thing (i.e. the loss of a loved one, see Maya Tamir's research on negative emotions can be useful). Or alternatively if we have a naive model that seeks to optimize HRV in the short term it will never recommend we exercise because that greatly decrease's one HRV in the moment even if in the long term exercise makes ones HRV increase. Using ML 
 ...
 I think the underlying issue that I'm now driving at is if we're not careful we'll end up with algorithms that treat us like machines, where you want all levels of everything to be in some optimal range at all times. But we're biological (complex) systems where physiology, and affect changes dynamically for good reason. I won't pretend I know all the reasons but these dynamic changes have developed over 1000 of years of evolution, so I'm willing to bet many of the reasons these things are good reasons.  So while of course there's potential for AI + Recommendation systems in precision psychiatry, the wearable, and wellness spaces developing these systmes will be difficult and need to be an iterative process, that goes hand and hand with science, rather than overly rigid emphasis on minimizing validation loss for a few easy to measure metrics.

 ### Implicit credits

As always we stand on the shoulders of giants. Although this repo is my own work a few people who have influenced my thinking on these ideas (although the things I get wrong are all my own fault): Ajay Satpute, Lisa Feldman Barrett, Karen Quigley, Peter Sterling, Eshin Jolly, Luke Chang, Johannes Jaeger, Marco Altini, and many many more. 
